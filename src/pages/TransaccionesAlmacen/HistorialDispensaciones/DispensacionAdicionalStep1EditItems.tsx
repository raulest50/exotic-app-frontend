...
import {LotePickerDispensacionAdicional} from './LotePickerDispensacionAdicional';
...
export default function DispensacionAdicionalStep1EditItems({
        ...
        return (
            <Box p='1em' bg='orange.50'>
                <Flex direction='column' gap={4} align='center'>
                    <Heading fontFamily='Comfortaa Variable' color='orange.700'>Dispensación Adicional - Materiales</Heading>
                    <Text fontFamily='Comfortaa Variable' fontSize='sm' color='gray.600'>
                        Seleccione los materiales y cantidades adicionales necesarios. Puede especificar cualquier cantidad mayor a 0.
                    </Text>
                    ...
                    {insumosEmpaque && insumosEmpaque.length > 0 && (
                        <>
                            ...
                            <Text fontFamily='Comfortaa Variable' fontSize='sm' color='gray.600'>
                                Materiales de empaque adicionales requeridos
                            </Text>
                            <Box bg='white' borderRadius='md' boxShadow='sm' overflowX='auto' w='full' maxW='1200px'>
                                <Table size='sm'>
                                    <Thead>
                    ...
                    <Flex w='40%' gap={4}>
                        ...
                        <Button 
                            ...
                            isDisabled={(() => {
                                // Validación flexible: solo requiere que haya al menos un material con lotes asignados
                                ...
                                // Obtener todos los materiales inventariables
                                ...
                                // Verificar si hay al menos un material con lotes asignados (receta o empaque)
                                const tieneLotesReceta = todosMaterialesInventariables.some(material => {
                                    const lotes = lotesPorMaterial.get(material.productoId);
                                    return lotes && lotes.length > 0 && lotes.some(l => l.cantidad > 0);
                                });
                                
                                const tieneLotesEmpaque = materialesEmpaqueInventariables.some(material => {
                                    const lotes = lotesPorMaterialEmpaque.get(material.productoId);
                                    return lotes && lotes.length > 0 && lotes.some(l => l.cantidad > 0);
                                });
                                
                                // Solo deshabilitar si no hay ningún lote asignado
                                return !tieneLotesReceta && !tieneLotesEmpaque;
                            })()}
                        >
                            Continuar
                        </Button>
                ...
                {modalAbierto && (
                    <LotePickerDispensacionAdicional
                        isOpen={true}
    ...
    return (
        <Box p='1em' bg='orange.50'>
            <Flex direction='column' gap={4} align='center'>
                <Heading fontFamily='Comfortaa Variable' color='orange.700'>Dispensación Adicional</Heading>
                <Text>No hay insumos disponibles para mostrar.</Text>
                <Flex w='40%' gap={4}>
                    <Button flex='1' onClick={()=>setActiveStep(0)}>Atrás</Button>
                    <Button flex='1' colorScheme='orange' onClick={()=>setActiveStep(2)}>Continuar</Button>
                </Flex>
            </Flex>
        </Box>
    );
