...
export function LotePickerDispensacionAdicional({
    isOpen,
    onClose,
    onAccept,
    productoId,
    ...
    // Validación flexible: solo verificar que haya al menos un lote con cantidad > 0
    const tieneLotesValidos = lotesSeleccionados.length > 0 && 
                               lotesSeleccionados.some(ls => ls.cantidad > 0);

    const handleAceptar = () => {
        if (tieneLotesValidos) {
            // Filtrar solo los lotes con cantidad > 0
            const lotesConCantidad = lotesSeleccionados.filter(ls => ls.cantidad > 0);
            onAccept(lotesConCantidad);
            onClose();
        }
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} size="6xl" isCentered>
            <ModalOverlay />
            <ModalContent>
                <ModalHeader>Definir Lotes Adicionales - {productoNombre}</ModalHeader>
                <ModalCloseButton />
                <ModalBody>
                    <Flex direction="column" gap={4}>
                        <Text fontSize="sm" color="gray.600">
                            Cantidad requerida original: <strong>{cantidadRequerida.toFixed(2)}</strong> (solo referencia)
                        </Text>
                        <Text fontSize="xs" color="orange.600" fontStyle="italic">
                            Puede especificar cualquier cantidad mayor a 0 para cada lote
                        </Text>
                                        ...
                                        <Box mt={4}>
                                            ...
                                            {!tieneLotesValidos && (
                                                <Text fontSize="xs" color="orange.500" mt={2}>
                                                    Debe asignar al menos una cantidad mayor a 0
                                                </Text>
                                            )}
                                        </Box>
                                    </>
